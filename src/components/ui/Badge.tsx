...
export function DecisionBadge({ decision, size = "md" }: DecisionBadgeProps) {
  return (
    <span
      className={`inline-flex items-center gap-1.5 font-semibold rounded-full border ${DECISION_STYLES[decision]} ${DECISION_SIZE[size]}`}
    >
...
export function TrustBadge({ level, size = "md" }: TrustBadgeProps) {
  return (
    <span
      className={`inline-flex items-center font-medium rounded-full border ${TRUST_STYLES[level]} ${size === "sm" ? "text-[10px] px-1.5 py-0.5" : "text-xs px-2 py-0.5"}`}
    >
      {level}
    </span>
  );
...
export function CriticalityBadge({
  ...
  return (
    <span
      className={`inline-flex items-center font-medium rounded-full border uppercase tracking-wide ${CRITICALITY_STYLES[level]} ${size === "sm" ? "text-[9px] px-1.5 py-0.5" : "text-[10px] px-2 py-0.5"}`}
    >
      {level}
    </span>
  );
...
export function AgentTypeBadge({ type }: AgentTypeBadgeProps) {
  ...
  return (
    <span
      className={`inline-flex items-center text-[10px] font-medium rounded-full border uppercase tracking-wide px-1.5 py-0.5 ${styles[type]}`}
    >
      {type}
    </span>
  );
...
export function SeverityBadge({ severity }: SeverityBadgeProps) {
  return (
    <span
      className={`inline-flex items-center text-[10px] font-semibold rounded-full border uppercase px-2 py-0.5 ${SEVERITY_STYLES[severity]}`}
    >
      {severity}
    </span>
  );
