...
function computeToolRisk(signals: ToolSignals): number {
  ...
  return snykRisk * 0.45 + sonarRisk * 0.25 + ravenRisk * 0.3;
}
...
function generateReasoning(
  ...
  if (snyk.status === "CRITICAL" || snyk.status === "WARNING") {
    findings.push(
      `Snyk detected ${snyk.highVulns > 0 ? `${snyk.highVulns} HIGH severity vulnerability` : `${snyk.medVulns} MEDIUM severity findings`}`,
    );
  }
  if (sonar.status === "WARNING" || sonar.status === "CRITICAL") {
    const issues = [];
    if (sonar.bugs > 0)
      issues.push(`${sonar.bugs} bug${sonar.bugs > 1 ? "s" : ""}`);
    if (sonar.codeSmells > 0)
      issues.push(
        `${sonar.codeSmells} code smell${sonar.codeSmells > 1 ? "s" : ""}`,
      );
    if (issues.length > 0)
      findings.push(`Sonar reported ${issues.join(" and ")}`);
  }
  if (raven.status === "VIOLATION") {
    findings.push(
      `Raven flagged policy violations: ${raven.policies.join(", ")}`,
    );
  }
...
function generateRecommendation(
  ...
  if (decision === "BLOCK") {
    const hasSecurityFindings =
      scenario.toolSignals.snyk.status === "CRITICAL" ||
      scenario.toolSignals.raven.status === "VIOLATION";
...
export const ANALYSIS_STEPS: AnalysisStep[] = [
  {
    id: "step-1",
    label: "Loading agent trust profile",
    detail: "Fetching historical behavior and trust score...",
    duration: 600,
  },
  {
    id: "step-2",
    label: "Collecting tool signals",
    detail: "Aggregating Snyk, SonarQube, and Raven results...",
    duration: 900,
  },
  {
    id: "step-3",
    label: "Computing risk matrix",
    detail: "Evaluating agent trust × tool risk × service criticality...",
    duration: 1100,
  },
  {
    id: "step-4",
    label: "Generating decision",
    detail: "Applying ARIA policy engine and generating explanation...",
    duration: 700,
  },
];
...
export function evaluateScenario(scenario: DemoScenario): EvaluationResult {
  ...
  const confidence = 0.7 + Math.abs(riskScore - 0.5) * 0.5;
