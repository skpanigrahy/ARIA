...
import { Zap, CheckCircle, Circle, AlertTriangle, Shield, GitPullRequest, Loader, ChevronDown } from 'lucide-react';
...
function ToolSignalRow({
  ...
  const label = isOk ? 'CLEAN' : isCritical ? (status === 'VIOLATION' ? 'VIOLATION' : 'CRITICAL') : 'WARNING';

  return (
    <div className="flex items-start gap-3 py-3 border-b border-[#21262D] last:border-0">
      <div className="w-5 h-5 rounded flex items-center justify-center flex-shrink-0 mt-0.5" style={{ backgroundColor: `${color}18` }}>
        {isOk ? <CheckCircle size={12} style={{ color }} /> : <AlertTriangle size={12} style={{ color }} />}
      </div>
      <div className="flex-1 min-w-0">
        <div className="flex items-center justify-between gap-2 mb-1">
          <span className="text-[#E6EDF3] text-xs font-medium">{name}</span>
          <span className="text-[10px] font-bold px-2 py-0.5 rounded-full border flex-shrink-0" style={{ color, borderColor: `${color}40`, backgroundColor: `${color}15` }}>
            {label}
          </span>
        </div>
        {finding && <p className="text-[#8B949E] text-[10px] leading-relaxed">{finding}</p>}
...
function AnalysisStepRow({ step, status }: {
  step: { id: string; label: string; detail: string };
  status: 'waiting' | 'running' | 'done';
}) {
  return (
    <div className={`flex items-center gap-3 py-2.5 transition-opacity duration-300 ${status === 'waiting' ? 'opacity-40' : 'opacity-100'}`}>
      ...
      <div>
        <p className={`text-sm font-medium ${status === 'waiting' ? 'text-[#656D76]' : 'text-[#E6EDF3]'}`}>{step.label}</p>
        {status === 'running' && (
          <p className="text-xs text-[#8B949E] mt-0.5 animate-pulse">{step.detail}</p>
        )}
      </div>
    </div>
  );
    ...
    <div className="space-y-5 animate-fadeIn">
      ...
      >
        <div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-[#E6EDF3] text-sm font-semibold">{result.pr.title}</p>
              <p className="text-[#8B949E] text-xs">{result.pr.serviceName} · <CriticalityBadge level={result.pr.criticality} size="sm" /></p>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="bg-[#161B22] border border-[#21262D] rounded-xl p-4">
          <p className="text-[#8B949E] text-xs font-medium uppercase tracking-wide mb-3">Agent at Evaluation</p>
          <div className="flex items-center gap-3">
            <div
            ...
            <div className="flex-1">
              <p className="text-[#E6EDF3] text-sm font-medium">{result.agent.name}</p>
              <TrustRing score={result.trustScoreAtTime} size={42} strokeWidth={4} showLabel={false} />
            </div>
            <div className="text-right">
              <p className="text-2xl font-bold tabular-nums" style={{ color: result.trustScoreAtTime >= 0.70 ? '#3FB950' : result.trustScoreAtTime >= 0.45 ? '#E3B341' : '#F85149' }}>
        ...
        <div className="bg-[#161B22] border border-[#21262D] rounded-xl p-4">
          <p className="text-[#8B949E] text-xs font-medium uppercase tracking-wide mb-3">Confidence</p>
          <p className="text-3xl font-bold text-[#58A6FF] tabular-nums">{Math.round(result.confidence * 100)}%</p>
          <p className="text-[#656D76] text-xs mt-1">ARIA confidence in this decision</p>
          <div className="mt-2 w-full h-1.5 bg-[#21262D] rounded-full overflow-hidden">
            <div className="h-full rounded-full bg-[#1F6FEB]" style={{ width: `${result.confidence * 100}%` }} />
          </div>
        </div>
      </div>

      <div className="bg-[#161B22] border border-[#21262D] rounded-xl p-4">
        <p className="text-[#8B949E] text-xs font-medium uppercase tracking-wide mb-4">Risk Breakdown</p>
        <div className="space-y-3">
          <RiskBar label="Agent Trust Risk" value={result.riskBreakdown.agentRisk} />
          <RiskBar label="Tool Signal Risk" value={result.riskBreakdown.toolRisk} />
          <RiskBar label="Service Criticality" value={result.riskBreakdown.criticalityRisk} />
        </div>
      </div>

      <div className="bg-[#161B22] border border-[#21262D] rounded-xl p-4">
        <p className="text-[#8B949E] text-xs font-medium uppercase tracking-wide mb-3">Tool Signals</p>
        <div>
          <ToolSignalRow
      ...
      <div className="rounded-xl p-4 border" style={{ backgroundColor: `${dc.bg}08`, borderColor: `${dc.border}30` }}>
        <p className="text-[#8B949E] text-xs font-medium uppercase tracking-wide mb-2">Recommendation</p>
    ...
    <div className="space-y-6">
      <div className="bg-gradient-to-r from-[#1F6FEB]/10 to-transparent border border-[#1F6FEB]/20 rounded-xl p-5">
        <div className="flex items-start gap-4">
          ...
          <div>
            <h2 className="text-[#E6EDF3] font-semibold text-base mb-1">ARIA Decision Engine</h2>
            <p className="text-[#8B949E] text-sm leading-relaxed">
              Select a pre-built scenario or provide custom PR data to see ARIA evaluate agent trust, tool signals, and service criticality in real time — producing a context-aware, explainable decision.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        <div className="space-y-4">
          <div>
            <label className="text-[#8B949E] text-xs font-medium uppercase tracking-wide mb-2 block">Select Demo Scenario</label>
            <div className="relative">
              ...
              >
                <div className="flex items-center gap-3">
                  <div className="w-2 h-2 rounded-full flex-shrink-0" style={{ backgroundColor: RISK_COLORS[selectedScenario.riskLevel] }} />
                  <div>
                    <p className="text-[#E6EDF3] text-sm font-medium">{selectedScenario.title}</p>
                    <p className="text-[#8B949E] text-xs">{selectedScenario.description}</p>
                  </div>
                </div>
                <ChevronDown size={16} className={`text-[#8B949E] transition-transform ${dropdownOpen ? 'rotate-180' : ''}`} />
              </button>
              {dropdownOpen && (
                <div className="absolute top-full left-0 right-0 mt-2 bg-[#161B22] border border-[#21262D] rounded-xl shadow-xl z-20 overflow-hidden">
                  {DEMO_SCENARIOS.map(scenario => (
                    ...
                    >
                      <div className="w-2 h-2 rounded-full flex-shrink-0 mt-0.5" style={{ backgroundColor: RISK_COLORS[scenario.riskLevel] }} />
                      <div>
                        <p className="text-[#E6EDF3] text-sm">{scenario.title}</p>
                        <p className="text-[#8B949E] text-xs">{scenario.description}</p>
                      </div>
                    </button>
                  ))}
                </div>
          ...
          <div className="bg-[#161B22] border border-[#21262D] rounded-xl p-4">
            <div className="flex items-center gap-2 mb-4">
              ...
              <p className="text-[#E6EDF3] text-xs font-semibold uppercase tracking-wide">PR Details</p>
            </div>
            <div className="space-y-2.5">
              {[
                ...
                { label: 'Criticality', value: <CriticalityBadge level={selectedScenario.pr.criticality} size="sm" /> },
                { label: 'Lines Changed', value: selectedScenario.pr.linesChanged.toLocaleString() },
                { label: 'Files Changed', value: selectedScenario.pr.filesChanged },
              ].map(item => (
                <div key={item.label} className="flex justify-between items-center py-1.5 border-b border-[#21262D]/70 last:border-0">
                  <span className="text-[#8B949E] text-xs">{item.label}</span>
                  <span className="text-[#E6EDF3] text-xs font-medium">{item.value}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          {agent && (
            <div className="bg-[#161B22] border border-[#21262D] rounded-xl p-4">
              <p className="text-[#8B949E] text-xs font-semibold uppercase tracking-wide mb-3">Submitting Agent</p>
              <div className="flex items-center gap-3">
                <div
                ...
                <div className="flex-1">
                  <p className="text-[#E6EDF3] text-sm font-medium">{agent.name}</p>
          ...
          <div className="bg-[#161B22] border border-[#21262D] rounded-xl p-4">
            <p className="text-[#656D76] text-xs italic leading-relaxed">{selectedScenario.demoNarrative}</p>
          </div>
        ...
        <div>
          {analysisState === 'idle' && (
            <div className="bg-[#161B22] border border-[#21262D] rounded-xl p-8 flex flex-col items-center justify-center min-h-64 text-center">
              ...
              <h3 className="text-[#E6EDF3] font-semibold mb-2">Ready to Evaluate</h3>
              <p className="text-[#8B949E] text-sm leading-relaxed max-w-xs">
                Select a scenario and click "Evaluate with ARIA" to see the full trust analysis, risk scoring, and decision reasoning.
              </p>
            </div>
          )}
          ...
          {analysisState === 'analyzing' && (
            <div className="bg-[#161B22] border border-[#21262D] rounded-xl p-6">
              <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                  <p className="text-[#E6EDF3] text-sm font-semibold">ARIA Analysis in Progress</p>
                  <p className="text-[#8B949E] text-xs">Processing multi-factor risk evaluation...</p>
                </div>
              </div>
              <div className="space-y-1">
                {ANALYSIS_STEPS.map((step, i) => (
                  <AnalysisStepRow
                    key={step.id}
                    step={step}
                    status={i < completedSteps ? 'done' : i === completedSteps ? 'running' : i === completedSteps + 1 ? 'running' : 'waiting'}
                  />
                ))}
              </div>
              <div className="mt-6 pt-4 border-t border-[#21262D]">
                <div className="w-full h-1 bg-[#21262D] rounded-full overflow-hidden">
                  <div
                    ...
                    style={{ width: `${((completedSteps + 1) / ANALYSIS_STEPS.length) * 100}%` }}
                  />
                </div>
                <p className="text-[#656D76] text-[10px] mt-2 text-center">
                  Step {Math.max(completedSteps + 1, 1)} of {ANALYSIS_STEPS.length}
                </p>
              </div>
            </div>
          )}
